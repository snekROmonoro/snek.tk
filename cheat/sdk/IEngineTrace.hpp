...
class Ray {
public:
	Ray( ) : m_world_axis_transform {} , m_is_ray {} , m_is_swept {} {}

	Ray( const vec3_t& start , const vec3_t& end ) {
	...
	Ray( const vec3_t& start , const vec3_t& end , const vec3_t& mins , const vec3_t& maxs ) {
		m_delta = vec_aligned_t( end - start );
		m_world_axis_transform = nullptr;
...
class ITraceFilter {
public:
	virtual bool        ShouldHitEntity( void* pEntity , int contents_mask ) = 0;
	virtual TraceType_t	GetTraceType( ) const = 0;
};

class CTraceFilter : public ITraceFilter {
public:
	bool ShouldHitEntity( void* entity , int contents_mask ) override {
...
public:
	...
	CTraceFilterSimple( const void* pass_ent1 , int collision_group = COLLISION_GROUP_NONE , ShouldHitFunc_t shouldhit_check_fn = nullptr ) :
	...
	virtual void SetPassEntity( void* pass_ent1 ) {
		m_pass_ent1 = pass_ent1;
	}
...
public:
	const void* m_pass_ent2;

public:
	CTraceFilterSkipTwoEntities( const void* pass_ent1 , const void* pass_ent2 , int collision_group = COLLISION_GROUP_NONE , ShouldHitFunc_t shouldhit_check_fn = nullptr ) :
	...
	virtual void SetPassEntity2( void* pass_ent2 ) {
		m_pass_ent2 = pass_ent2;
	}
};
...
public:
	surfacedata_t* GetSurfaceData( int surfaceDataIndex ) {
		return util::get_virtual_function< surfacedata_t* ( __thiscall* )( void* , int ) >( this , 5 )( this , surfaceDataIndex );
	}
};

class IEngineTrace {
public:
	int GetPointContents( const vec3_t& position , int mask = MASK_ALL , void** entity = nullptr ) {
		return util::get_virtual_function< int( __thiscall* )( void* , const vec3_t& , int , void** ) >( this , 0 )( this , position , mask , entity );
	}

	void ClipRayToEntity( const Ray& ray , uint32_t mask , void* entity , CGameTrace* trace ) {
		return util::get_virtual_function< void( __thiscall* )( void* , const Ray& , uint32_t , void* , CGameTrace* ) >( this , 3 )( this , ray , mask , entity , trace );
	}

	void TraceRay( const Ray& ray , uint32_t mask , ITraceFilter* filter , CGameTrace* trace ) {
		return util::get_virtual_function< void( __thiscall* )( void* , const Ray& , uint32_t , ITraceFilter* , CGameTrace* ) >( this , 5 )( this , ray , mask , filter , trace );
	}
};