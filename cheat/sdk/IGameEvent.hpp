...
#include "../util/modules/modules.hpp"
...
public:
	...
	static __forceinline T KeyValuesSystem( ) {
		static auto KeyValuesFactory = util::modules::GetExport( util::modules::GetModule( "vstdlib.dll" ) , "KeyValuesSystem" );
		return KeyValuesFactory.get< T( * )( ) >( )( );
	}
};
...
public:
	KeyValues* FindKey( std::string name ) {
		KeyValues* dat;
	...
	int GetInt( int default_value = 0 ) {
		int result {};
	...
	float GetFloat( float default_value = 0.f ) {
		switch ( this->m_data_type ) {
		case TYPE_STRING:
