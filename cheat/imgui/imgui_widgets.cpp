...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    if (!(flags & ImGuiComboFlags_NoPreview))
        window->DrawList->AddRectFilled(frame_bb.Min, /*ImVec2(value_x2, frame_bb.Max.y)*/frame_bb.Max, frame_col, style.FrameRounding, (flags & ImGuiComboFlags_NoArrowButton) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Left);
    ...
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
        RenderTextClipped(frame_bb.Min + style.FramePadding, ImVec2(value_x2, frame_bb.Max.y), preview_value, NULL, NULL, ImVec2(0.f,0.f));
    if (label_size.x > 0)
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, float power)
{
    ...
    const bool hovered = ItemHoverable(frame_bb, id);
#ifdef ALLOW_DEAR_IMGUI_TO_KEYINPUT_SLIDERS
    bool temp_input_is_active = TempInputIsActive(id);
#else
    ...
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    //RenderTextClipped(frame_bb.Min, frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.5f,0.5f));
    RenderTextClipped(frame_bb.Min, ImVec2(frame_bb.Max.x - style.ItemInnerSpacing.x, frame_bb.Max.y), value_buf, value_buf_end, NULL, ImVec2(label_size.x > 0.0f ? 1.f : 0.5f,0.5f));
