        ...
        if (g.IO.MouseDown[0] && IsMousePosValid(&g.IO.MousePos))
        {
            ...
            if (moving_window->Pos.x != pos.x || moving_window->Pos.y != pos.y)
            {
                ...
                /* clamp window pos on screen */ {
                    if ( moving_window->Pos.x < 0 )
                        moving_window->Pos.x = 0;

                    if ( moving_window->Pos.y < 0 )
                        moving_window->Pos.y = 0;

                    if ( ( moving_window->Pos.x + moving_window->Size.x ) > g.IO.DisplaySize.x )
                        moving_window->Pos.x = g.IO.DisplaySize.x - moving_window->Size.x;

                    if ( ( moving_window->Pos.y + moving_window->Size.y ) > g.IO.DisplaySize.y )
                        moving_window->Pos.y = g.IO.DisplaySize.y - moving_window->Size.y;
                }
            }
...
static void ImGui::NavUpdateWindowing()
{
    ...
    if (g.NavWindowingTarget && !(g.NavWindowingTarget->Flags & ImGuiWindowFlags_NoMove))
    {
        ...
        if (move_delta.x != 0.0f || move_delta.y != 0.0f)
        {
            ...
            /* clamp window pos on screen */ {
                if ( g.NavWindowingTarget->RootWindow->Pos.x < 0 )
                    g.NavWindowingTarget->RootWindow->Pos.x = 0;

                if ( g.NavWindowingTarget->RootWindow->Pos.y < 0 )
                    g.NavWindowingTarget->RootWindow->Pos.y = 0;

                if ( ( g.NavWindowingTarget->RootWindow->Pos.x + g.NavWindowingTarget->RootWindow->Size.x ) > g.IO.DisplaySize.x )
                    g.NavWindowingTarget->RootWindow->Pos.x = g.IO.DisplaySize.x - g.NavWindowingTarget->RootWindow->Size.x;

                if ( ( g.NavWindowingTarget->RootWindow->Pos.y + g.NavWindowingTarget->RootWindow->Size.y ) > g.IO.DisplaySize.y )
                    g.NavWindowingTarget->RootWindow->Pos.y = g.IO.DisplaySize.y - g.NavWindowingTarget->RootWindow->Size.y;
            }
